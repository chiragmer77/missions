...
@Component({
  selector: 'app-projectCategory-add',
  templateUrl: './add.component.html',
  styleUrls: ['./add.component.css']
})
export class AddProjectCategoryComponent {
  ...
  onFormSubmit() {
    ...
    if (this.myForm!.valid) {
      ...
      if (this.isEditing) {
        this.apiService.put(`ProjectCategory/${payload.id}`, payload).subscribe((response) => {
          if (response.success) {
        ...
        this.spinner.show();
        delete payload.id;
        this.apiService.post('ProjectCategory', payload).subscribe((response) => {
          if (response.success) {
